...
// Step 1: Handle values with regex (add quotes to string values)
jsonString = jsonString.replace(/(\w+)=([^,{}]+)/g, function(match, key, value) {
    ...
    if (value === 'null' || value === '') {
        ...
        // If the value is a number or boolean, don't add quotes
        return `${key}=${value}`;
    } else {
        // Otherwise, it's a string, so wrap it in quotes
        return `${key}="${value}"`;
    }
});

// Step 3: Replace `=` with `:` only after processing everything
