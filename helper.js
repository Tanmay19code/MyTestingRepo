...
// Step 1: Handle `${...}` placeholders by wrapping them in quotes (keep the `$` and `{}` intact)
jsonString = jsonString.replace(/\${([^}]*)}/g, (match) => {
    return `"${match}"`;  // Wrap the whole `${...}` as a string
});

// Step 2: Handle the rest of the key-value pairs and process values
jsonString = jsonString.replace(/(\w+)=([^,{}]*)/g, function(match, key, value) {
    value = value.trim(); // Trim any extra spaces
    ...
    if (value === '') {
        ...
        // If the value is already wrapped in quotes, don't add quotes again
        return `"${key}":${value}`;
    } else {
        // Otherwise, it's a string, so wrap the value in quotes
        return `"${key}":"${value}"`;
...
jsonString = jsonString.replace(/(\w+)=([^,{}]*)/g, function(match, key, value) {
    ...
    if (value === '') {
        ...
        // If the value is already wrapped in quotes (like in the case of ${...}), don't add quotes again
        return `"${key}":${value}`;
    } else {
        // Otherwise, it's a string, so wrap the value in quotes
        return `"${key}":"${value}"`;
